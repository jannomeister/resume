...
const Resume = (props) => {
      ...
      <div>
        ...
        <ul className="bgrid-quarters s-bgrid-thirds cf">
          {items.map((item, index) => (
            <div
              key={item.name + index}
              className="columns feature-item"
              style={{
                textAlign: "center",
                marginTop: "2.5em",
                paddingLeft: "2em",
            ...
            >
              <img
                className="skill"
                style={{ maxWidth: "65%" }}
                src={`images/favorite_tech/${item.image}`}
                alt={item.name}
              />
              <h5>{item.name}</h5>
            ...
            {items.map((item, index) => (
              <li key={item.name + index}>
                <span
                  ...
                  className={`bar-expand ${item.name.toLowerCase()}`}
                ></span>
                <em>{item.name}</em>
              </li>
            ))}
  ...
  return (
    <section id="resume">
      ...
      {props.data && (
        <FavoriteTech
          message={props.data.techMessage}
          items={props.data.favoriteTech}
        />
      )}
      {props.data && (
        <Skill message={props.data.skillmessage} items={props.data.skills} />
      )}
    </section>
  );
};
